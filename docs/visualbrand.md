...
## Creating Visual Assets for Social Media

### SVG Creation

Claude can create SVG visual assets directly. When creating visuals:
- Use dimensions 1200x630 for LinkedIn (optimal for feed)
- Apply the Dusk palette colors
- Use Inter for body text, Georgia (serif) as fallback for Zilla Slab
- Keep designs clean and scannable
- Include MVP Club attribution

### Converting SVG to PNG

**LinkedIn and most social platforms don't accept SVG files.** Convert to PNG before posting.

**Script location:** `/scripts/svg-to-png.js`

**Usage:**
```bash
cd /mnt/c/Users/mhast/Desktop/mvpclubmarketing/scripts
node svg-to-png.js path/to/image.svg [optional-output.png]
```

**Examples:**
```bash
# Convert and save as same name with .png extension
node svg-to-png.js ../marketing/to-post/linkedin/matt/visual.svg

# Convert with custom output name
node svg-to-png.js ../marketing/to-post/linkedin/matt/visual.svg output.png
```

The script:
- Reads SVG dimensions automatically
- Renders at exact size (no scrollbars or whitespace)
- Outputs clean PNG ready for upload

### Visual Asset Workflow

1. **Create SVG** → Claude generates SVG in `/to-review/`
2. **Review** → Check visual matches brand guidelines
3. **Approve** → Move to `/to-post/`
4. **Convert** → Run `svg-to-png.js` to create PNG
5. **Post** → Upload PNG to social platform
